...
def mu_find_and_integrate(integrator, mu_in, y_in, nelec, beta, alpha, h1):
    """
    ...
    if ndiff_mag > ntol:
        ...
        while np.sign(num - nelec) == ndiff_sgn:
            ...
            integrator.set_f_params(beta, alpha, h1)
        ...
        while np.abs(num - nelec)>ntol:
            ...
            integrator.set_initial_value(yf1, mu_bisect[0]).set_f_params(beta, alpha, h1)
        ...
        mu_f = mu_mid
        integrator.set_initial_value(yf, mu_0).set_f_params(beta, alpha, h1)
